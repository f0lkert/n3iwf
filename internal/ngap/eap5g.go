...
func UnmarshalEAP5GData(
	...
	anParameters, err = func() (*ANParameters, error) {
		// An AN-parameter length of zero is valid, which indicates no AN-parameters included in EAP-5G packet.
		if anParameterLength == 0 {
			ngapLog.Debug("No AN-parameters included in this EAP-5G packet")
			return nil, nil
		}
		...
		if len(anParameterField) < int(anParameterLength) {
			ngapLog.Error("Packet contained error length of value")
			return nil, errors.New("error formatting")
		}
		anParameterField = anParameterField[:anParameterLength]

		ngapLog.Debugf("Parsing AN-parameters...: % v", anParameterField)

		params := new(ANParameters)
		...
		for len(anParameterField) >= 2 {
			...
			if int(2+parameterLength) > len(anParameterField) {
				...
				return nil, errors.New("malformed AN-parameter with parameter length exceeds remaining data bounds")
			}
			...
			case message.ANParametersTypeGUAMI:
				...
				if parameterLength != 0 {
					...
					if len(parameterValue) != message.ANParametersLenGUAMI {
						return nil, errors.New("unmatched GUAMI length")
					}
					...
					params.GUAMI = ngapGUAMI
					ngapLog.Debugf("Unmarshal GUAMI: % x", guamiField)
					ngapLog.Debugf("\tGUAMI: PLMNIdentity[% x], "+
						"AMFRegionID[% x], AMFSetID[% x], AMFPointer[% x]",
						params.GUAMI.PLMNIdentity, params.GUAMI.AMFRegionID,
						params.GUAMI.AMFSetID, params.GUAMI.AMFPointer)
				} else {
			...
			case message.ANParametersTypeSelectedPLMNID:
				...
				if parameterLength != 0 {
					...
					if len(parameterValue) != message.ANParametersLenPLMNID {
						return nil, errors.New("unmatched PLMN ID length")
					}
					...
					params.SelectedPLMNID = ngapPLMN
					ngapLog.Debugf("Unmarshal SelectedPLMNID: % x", plmnField)
					ngapLog.Debugf("\tSelectedPLMNID: % x", params.SelectedPLMNID.Value)
				} else {
			...
			case message.ANParametersTypeRequestedNSSAI:
				...
				if parameterLength != 0 {
					...
					params.RequestedNSSAI = ngapNSSAI
				} else {
			...
			case message.ANParametersTypeEstablishmentCause:
				...
				if parameterLength != 0 {
					...
					if len(parameterValue) != message.ANParametersLenEstCause {
						return nil, errors.New("unmatched Establishment Cause length")
					}
					...
					params.EstablishmentCause = ngapEstablishmentCause
				} else {
					ngapLog.Warn("AN-Parameter establishment cause field empty")
				}
			default:
				ngapLog.Warn("Unsupported AN-Parameter. Ignore.")
			}
	...
	// If error occurs when parsing AN-parameters, return the error. Otherwise, continue to parse NAS PDU.
	if err != nil {
		return nil, nil, err
	}
